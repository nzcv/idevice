...
class Devicectl(DeviceBase):
    """Implement the IOSDevice5 operations exposed by CoreDevice.

    This layer only invokes Apple's ``xcrun devicectl`` and never performs its
    own ios4 fallback. The IOSDevice5 facade owns routing between backends.
    ...
    * ``device copy to`` / ``device copy from`` and ``device info files`` for
    ...
    Unsupported operations raise :class:`NotImplementedError`; the facade
    routes those directly to its ios4 layer.
    """
    ...
    def __init__(
        ...
        *,
        ...
        if validate_host and sys.platform != "darwin":
        ...
        if validate_host and self._resolve_binary(self._xcrun) is None:
    ...
    @classmethod
    def from_env(cls) -> Devicectl:
        """Build a :class:`Devicectl` from the ``GAUTO_*`` environment."""
        return cls(
            env_device_id(),
    ...
    def _processes(self) -> list[dict[str, Any]]:
        ...
        Raises:
            IOSDevice5Error: If devicectl cannot read the process table.
        """
    ...
    def _terminate(self, pid: int, *, kill: bool = True) -> bool:
        ...
        self._require(outcome, f"terminate PID {pid}")
        return True
    ...
    def capture_memgraph(
        ...
        del output, pid
        self._unsupported("capture_memgraph")
    ...
    def _screenshot_via_ios4(self, local_path: Path) -> bool:
        del local_path
        self._unsupported("ios4 screenshot fallback")
    ...
    def _list_container(
        ...
        Raises:
            IOSDevice5Error: If devicectl cannot read the directory.
        """
    ...
    def start_moniter(self, duration: int = 180) -> bool:
        del duration
        self._unsupported("start_moniter")

    def stop_moniter(self) -> bool:
        self._unsupported("stop_moniter")
