"""Public ``Record`` entry point for iRecord-backed screen-recording control.

Use :meth:`Record.create` / :meth:`Record.from_env` to build a recorder:
``macos`` yields a real :class:`~idevice.record.mac.record.MacRecord`; every
...
class Record(metaclass=_RecordMeta):
        ...
        *,
        ...
        Args:
            record_type: Target host type (``macos`` runs the iRecord-backed
                recorder; every other value resolves to a :class:`DummyRecord`).
            server_ip: iRecord control-server IP (the mac host).
        ...
        Raises:
            ValueError: If ``record_type`` is unsupported, or (for ``macos``) a
    ...
    def from_env(cls) -> RecordBase:
        ...
        coordinate logs the reason and returns a no-op :class:`DummyRecord`. The
        result (real or dummy) is bound as :attr:`Record.Instance`, so callers
        can always reach it there.

        Returns:
            RecordBase: The host-type-specific recorder, or a no-op
