...
- **CI:** `.github/workflows/workflow.yml` — push to `main`/`master` (this repo uses `master`) runs test + lint, then builds and publishes to PyPI via trusted publishing (`environment: pypi`)
...
1. Confirm GitHub Actions **CI** workflow started on the pushed branch (`master`).
...
Use tags for release tracking; PyPI publish is already triggered by `master` pushes.
...
| Test | `uv run pytest --import-mode=importlib` |
